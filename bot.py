...
async def start(update: Update, context):
    await update.message.reply_text(
        "🤖 *Izdihom AI Media Bot*\n\nQuyidagi menyudan xizmat yoki paket tanlang.\nTelefon raqam va izoh qoldiring, operatorlar bog‘lanadi.\n\n📞 Admin: @Izdihom_MG",
...
async def button_handler(update: Update, context):
    ...
    if data == "main_menu":
        ...
        await query.edit_message_text("🎨 *AI Dizayn*", reply_markup=InlineKeyboardMarkup(kb), parse_mode="Markdown")
        ...
        kb = [[InlineKeyboardButton(f"8 sek video – {prices['video8']}", callback_data="video8")],
              ...
              [InlineKeyboardButton("◀️ Orqaga", callback_data="main_menu")]]
        await query.edit_message_text("🎬 *AI Video*", reply_markup=InlineKeyboardMarkup(kb), parse_mode="Markdown")
        ...
        kb = [[InlineKeyboardButton(f"START – {prices['start']}", callback_data="start")],
              ...
              [InlineKeyboardButton("◀️ Orqaga", callback_data="main_menu")]]
        await query.edit_message_text("📦 *Oylik paketlar*", reply_markup=InlineKeyboardMarkup(kb), parse_mode="Markdown")
    elif data == "help":
        await query.edit_message_text("Yordam:\n1. Xizmat tanlang\n2. Telefon raqam\n3. Izoh qoldiring\nOperator bog‘lanadi.", reply_markup=main_menu())
    else:
        context.user_data['selected'] = data
        await query.edit_message_text(
            f"✅ Siz tanladingiz: *{descriptions[data]}* – {prices[data]}\n\n📞 Endi telefon raqamingizni yuboring (+998901234567):\nBekor qilish uchun /cancel",
            parse_mode="Markdown"
        )
        return PHONE
...
async def comment_handler(update: Update, context):
    ...
    text = (f"🆕 *YANGI BUYURTMA (bot)* 🆕\n\n"
            ...
            f"📞 Telefon: {phone}\n"
            f"💬 Izoh: {comment}\n"
            f"👤 Foydalanuvchi: @{update.effective_user.username or 'anonim'}\n"
